...
import com.tradeflow.journal.ai.AgentCommentService
...
class TradeViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val aiApiKey: StateFlow<String>
    val aiBaseUrl: StateFlow<String>
    val aiModel: StateFlow<String>
    val aiAutoGenerate: StateFlow<Boolean>
    ...
    init {
        ...
        aiProvider = preferencesRepository.aiProvider.stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5000),
            initialValue = "DeepSeek"
        )
        aiApiKey = preferencesRepository.aiApiKey.stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5000),
            initialValue = ""
        )
        aiBaseUrl = preferencesRepository.aiBaseUrl.stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5000),
            initialValue = "https://api.deepseek.com/v1"
        )
        aiModel = preferencesRepository.aiModel.stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5000),
            initialValue = "deepseek-chat"
        )
        aiAutoGenerate = preferencesRepository.aiAutoGenerate.stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5000),
            initialValue = false
        )
    ...
    private fun buildAgent(): AgentCommentService? {
        val key = aiApiKey.value
        val url = aiBaseUrl.value
        val model = aiModel.value
        if (key.isBlank() || url.isBlank() || model.isBlank()) return null
        return AgentCommentService(apiKey = key, baseUrl = url, model = model)
    }

    /**
     * Generate (or regenerate) the AI comment for one trade and persist it.
     * Returns true on success.
     */
    fun regenerateNotes(trade: Trade) {
        viewModelScope.launch(Dispatchers.IO) {
            val agent = buildAgent() ?: run {
                viewModelScope.launch(Dispatchers.Main) {
                    android.widget.Toast.makeText(
                        getApplication(),
                        "Set the AI agent API key in Settings first",
                        android.widget.Toast.LENGTH_LONG
                    ).show()
                }
                return@launch
            }
            val reason = agent.generateReason(trade)
            if (reason != null) {
                val updated = trade.copy(notes = reason)
    ...
    /**
     * Quick sanity check: build a fake trade and ask the agent to comment on
     * it. Returns true if the agent returned non-blank text.
     */
    suspend fun testAiConnection(): Boolean {
        val agent = buildAgent() ?: return false
        val sample = Trade(
            symbol = "BTC/USDT",
            side = TradeSide.LONG,
            entryPrice = 60000.0,
            exitPrice = 61200.0,
            quantity = 0.0166,
            entryTime = System.currentTimeMillis() - 3_600_000L,
        ...
        val reason = agent.generateReason(sample)
        return !reason.isNullOrBlank()
    }

    private suspend fun maybeAutoFillNotes(savedId: Long) {
        if (!aiAutoGenerate.value) return
        val agent = buildAgent() ?: return
        val saved = repository.getTradeById(savedId) ?: return
        val current = saved.notes
        val isPlaceholder = current.isBlank() ||
            current.startsWith("Imported from CSV") ||
            current.startsWith("Synced from Server")
        if (!isPlaceholder) return
        val reason = agent.generateReason(saved) ?: return
        repository.updateTrade(saved.copy(notes = reason))
    }
    ...
    fun importTradesFromCsv(uri: android.net.Uri) {
        viewModelScope.launch {
            ...
            try {
                ...
                // --- AI AUTO-COMMENT (background) ---
                if (aiAutoGenerate.value && aiApiKey.value.isNotBlank() && savedIds.isNotEmpty()) {
                    viewModelScope.launch(Dispatchers.IO) {
                        savedIds.forEach { id ->
                            try {
                                maybeAutoFillNotes(id)
                            } catch (e: Exception) {
                                android.util.Log.e("TradeViewModel", "AI fill failed for $id", e)
                            }
                        }
                        viewModelScope.launch(Dispatchers.Main) {
                            android.widget.Toast.makeText(
                                getApplication(),
                                "AI agent finished writing comments",
                                android.widget.Toast.LENGTH_SHORT
                            ).show()
                        }
                    }
        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            ...
            try {
                ...
                if (connection.responseCode == 200) {
                    ...
                    while (line != null) {
                        try {
                            val savedId = parseAndSaveTrade(line, existingSignatures)
                            if (savedId != null) {
                                newIds.add(savedId)
                                importedCount++
                            } else {
                    ...
                    // --- AI AUTO-COMMENT (background) ---
                    if (aiAutoGenerate.value && aiApiKey.value.isNotBlank() && newIds.isNotEmpty()) {
                        newIds.forEach { id ->
                            try {
                                maybeAutoFillNotes(id)
                            } catch (e: Exception) {
    ...
    // --- AI savers ---
    fun saveAiProvider(value: String) {
        viewModelScope.launch { preferencesRepository.saveAiProvider(value) }
    }
    fun saveAiApiKey(value: String) {
        viewModelScope.launch { preferencesRepository.saveAiApiKey(value) }
    }
    fun saveAiBaseUrl(value: String) {
        viewModelScope.launch { preferencesRepository.saveAiBaseUrl(value) }
    }
    fun saveAiModel(value: String) {
        viewModelScope.launch { preferencesRepository.saveAiModel(value) }
    }
    fun saveAiAutoGenerate(value: Boolean) {
        viewModelScope.launch { preferencesRepository.saveAiAutoGenerate(value) }
    }
    ...
    /**
     * Parses one CSV line and inserts the trade. Returns the new row id if
     * inserted, or null if skipped (duplicate or unparseable).
     */
    private suspend fun parseAndSaveTrade(line: String, existingSignatures: HashSet<String>): Long? {
        val tokens = line.split(",")
        ...
        return repository.insertTrade(trade)
    }
